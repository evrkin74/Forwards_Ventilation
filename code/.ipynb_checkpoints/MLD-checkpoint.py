...
'''
import glob
#open a dataset of MLD between 1982 and 2012
model_paths = []
min_yr=2000
max_yr=2000 ##inclusive
...
#open one file
path1=model_paths[0]
ds_arr=xr.open_dataset(path1)
print(ds_arr.dims) 


ds_mld = xr.open_mfdataset(model_paths, chunks='auto', parallel=True)
ds_test = ds_mld.isel(time_counter=0).nav_lon.values
print(ds_test)

if 'x' not in ds_mld.coords or 'y' not in ds_mld.coords:
    raise ValueError("The dataset does not have coordinates for 'x' and 'y'.")

#need to calculate a time_counter for the year_o, month_o in df_vent
df_vent=df_vent[(df_vent['year_o']>=min_yr)&(df_vent['year_o']<=max_yr)]
df_vent['time_counter'] = (df_vent['year_o'] - min_yr)*12 + df_vent['month_o']

print(len(df_vent))
t1=time.time()
# Find the MLD at the vent time in one row, then apply over whole dataset
def find_mld(df_vent, ds_mld):
    df_vent['mld'] = df_vent.apply(lambda row: ds_mld.mldr10_1.sel(x=row['binnedx_o'],y=row['binnedy_o'],method='nearest').isel(time_counter=row['time_counter']).values, axis=1)
    return df_vent


df_vent = df_vent.map_partitions(find_mld, ds_mld=ds_mld,meta={'mld':'float64'})
df_vent = df_vent.compute()
# store now and do some time experiments to see efficiency
df_vent.to_parquet(f'../data/MLD/df_vent_test.parquet')
t2=time.time()
print(t2-t1)
print(df_vent.head())
